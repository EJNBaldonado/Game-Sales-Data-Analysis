...
CONSOLES = ["nintendo-ds", "super-nintendo", "nes", "nintendo-64", "gamecube", "wii", "wii-u", "nintendo-switch", "gameboy",
        "gameboy-advance"]

''' 
, "nes", "nintendo-64", "gamecube", "wii", "wii-u", "nintendo-switch", "gameboy",
...
# produce exception "Failes to decode response from marionette" in main()
...
def gameCsv(games):
    ...
    '''
    # Converts current datetime to string format
    dt = datetime.now().strftime("%d.%m.%Y_%H.%M.%S")

    fn = "game_prices.csv"
    # Check if the path exists 
    if not os.path.exists(dt):
        # If it does not exist create a path to the datetime
        os.mkdir(dt)
    # with - Cleaner aproach to file open / file close for resource mngment/exception handling
    # open - opens a file path
    # join - joins one or more path components intelligently (concatenates with / dir separator) 
    with open(os.path.join(dt, fn), 'w') as csv_file:
        fieldnames = ['game', 'console', 'loose_val', 'complete_val', 'new_val', 'date(D/M/Y)']
        # writes name of file w fieldnames
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        # writes headers to the file object
        writer.writeheader()
        # For each game in games write a row with info
        for g in games:
            writer.writerow({'game': g.getTitle(), 'console': g.getConsole(), 'loose_val': g.getLoosePrice(), 
                             'complete_val': g.getCompletePrice(), 'new_val': g.getNewPrice(), 'date(D/M/Y)': dt.split('_')[0].replace('.','/')})
            
def scrollBottom(console):
    '''Scrolls to the bottom of webpage. pricecharting.come/console/<console-name> loads x number of videogames at a time, this loads all videogames
    for our console before we scrape values
...
def scrapeVals(console, browser):
    ...
    Args:
        console: (string) game system name as it appears in the pricecharting URL
        browser: (brwoser) html for webpage with all values loaded
...
def pullVals(console):
    '''Pull values from pricecharting.com
